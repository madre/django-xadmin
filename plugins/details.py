    ...
    def result_item(self, item, obj, field_name, row):
        if (self.show_all_rel_details or (field_name in self.show_detail_fields)):
            ...
            if field_name in self.show_detail_fields:
                    if isinstance(self.show_detail_fields, dict):
                        if self.show_detail_fields[field_name]:
                            rel_obj = getattr(obj, self.show_detail_fields[field_name])
                ...
                try:
                    ...
                    if item_res_uri:
                        ...
                        item.btns.append('<a data-res-uri="%s" data-edit-uri="%s" class="details-handler" rel="tooltip" title="%s"><i class="fa fa-info-circle"></i></a>'
                                         % (item_res_uri, edit_url, _(u'%s') % str(rel_obj)))
